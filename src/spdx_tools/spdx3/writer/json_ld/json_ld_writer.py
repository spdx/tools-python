# SPDX-FileCopyrightText: 2023 spdx contributors
#
# SPDX-License-Identifier: Apache-2.0
import json
import os

from spdx_tools.spdx3.payload import Payload
from spdx_tools.spdx3.writer.json_ld.json_ld_converter import convert_payload_to_json_ld_list_of_elements

def convert_payload_to_dict(payload: Payload, inline_context: bool = True) -> dict:
    element_list = convert_payload_to_json_ld_list_of_elements(payload)

    # this will be obsolete as soon as the context is publicly available under some URI
    if inline_context:
        with open(os.path.join(os.path.dirname(__file__), "context.json"), "r") as infile:
            context = json.load(infile)["@context"]
    else:
        context = "https://spdx.github.io/spdx-3-model/context.json"

    return {"@context": context, "@graph": element_list}

def write_payload(payload: Payload, file_name: str, inline_context: bool = True) -> None:
    complete_dict = convert_payload_to_dict(payload, inline_context)

    with open(file_name + ".jsonld", "w") as out:
        json.dump(complete_dict, out, indent=2)

def payload_to_jsonld_string(payload: Payload, inline_context: bool = True) -> str:
    complete_dict = convert_payload_to_dict(payload, inline_context)

    return json.dumps(complete_dict, indent=2)
