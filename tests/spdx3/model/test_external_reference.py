...
def test_correct_initialization():
    external_reference = ExternalReference(ExternalReferenceType.SECURITY_ADVISORY, ["https://anyURI"], "MediaType",
                                           "This is a comment")
...
def test_invalid_initialization():
    ...
    assert err.value.args[0] == ['SetterError ExternalReference: type of argument "external_reference_type" '
                                 'must be one of (spdx3.model.external_reference.ExternalReferenceType, '
                                 'NoneType); got str instead: OTHER',
                                 'SetterError ExternalReference: type of argument "locator" must be a list; '
                                 'got str instead: a URI',
