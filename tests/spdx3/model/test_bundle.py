...
def test_correct_initialization(creation_information, namespace):
    ...
    assert bundle.element == ["spdx_id1"]
    assert bundle.root_element == ["spdx_id2"]
...
def test_invalid_initialization(creation_information):
    with pytest.raises(TypeError) as err:
        Bundle(4, creation_information, element="spdx_id1", root_element=[42], namespaces=True, context=["yes"])

    assert err.value.args[0] == [
        'SetterError Bundle: type of argument "spdx_id" must be str; got int instead: 4',
        'SetterError Bundle: type of argument "element" must be a list; got str ' "instead: spdx_id1",
        'SetterError Bundle: type of argument "root_element"[0] must be str; got int ' "instead: [42]",
