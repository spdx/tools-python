...
def test_tokenization_of_external_document_references(lexer):
    ...
    token_assert_helper(lexer.token(), 'DOC_REF_ID', 'DocumentRef-spdx-tool-2.1', 2)
    token_assert_helper(lexer.token(), 'DOC_URI', 'http://spdx.org/spdxdocs/spdx-tools-v2.1-3F25'
...
def test_tokenization_of_file(lexer):
    ...
    token_assert_helper(lexer.token(), 'SOURCE', 'SOURCE', 3)
    token_assert_helper(lexer.token(), 'FILE_TYPE', 'FileType', 4)
    token_assert_helper(lexer.token(), 'FILETYPE_TEXT', 'TEXT', 4)
    ...
    token_assert_helper(lexer.token(), 'FILE_LICS_CONC', 'LicenseConcluded', 6)
    token_assert_helper(lexer.token(), 'LINE', 'Apache-2.0', 6)
    token_assert_helper(lexer.token(), 'FILE_LICS_INFO', 'LicenseInfoInFile', 7)
    token_assert_helper(lexer.token(), 'LINE', 'Apache-2.0', 7)
    token_assert_helper(lexer.token(), 'FILE_CR_TEXT', 'FileCopyrightText', 8)
...
def test_tokenization_of_creation_info(lexer):
    ...
    token_assert_helper(lexer.token(), 'ORG_VALUE', 'Organization: Acme.', 2)
...
def test_tokenization_of_package(lexer):
    ...
    token_assert_helper(lexer.token(), 'PKG_DOWN', 'PackageDownloadLocation', 4)
    ...
    token_assert_helper(lexer.token(), 'PKG_SUM', 'PackageSummary', 6)
    token_assert_helper(lexer.token(), 'TEXT', '<text>Test package</text>', 6)
    token_assert_helper(lexer.token(), 'PKG_SRC_INFO', 'PackageSourceInfo', 7)
    ...
    token_assert_helper(lexer.token(), 'PKG_SUPPL', 'PackageSupplier', 9)
    token_assert_helper(lexer.token(), 'ORG_VALUE', 'Organization:ACME', 9)
    token_assert_helper(lexer.token(), 'PKG_ORIG', 'PackageOriginator', 10)
    token_assert_helper(lexer.token(), 'ORG_VALUE', 'Organization:ACME', 10)
    ...
    token_assert_helper(lexer.token(), 'PKG_VERF_CODE', 'PackageVerificationCode', 12)
    token_assert_helper(lexer.token(), 'LINE',
                        '4e3211c67a2d28fced849ee1bb76e7391b93feba (something.rdf, something.txt)', 12)
    token_assert_helper(lexer.token(), 'PKG_DESC', 'PackageDescription', 13)
    ...
    token_assert_helper(lexer.token(), 'PKG_CPY_TEXT', 'PackageCopyrightText', 15)
    token_assert_helper(lexer.token(), 'TEXT', '<text> Copyright 2014 Acme Inc.</text>', 15)
    token_assert_helper(lexer.token(), 'PKG_LICS_DECL', 'PackageLicenseDeclared', 16)
    token_assert_helper(lexer.token(), 'LINE', 'Apache-2.0', 16)
    token_assert_helper(lexer.token(), 'PKG_LICS_CONC', 'PackageLicenseConcluded', 17)
    token_assert_helper(lexer.token(), 'LINE', '(LicenseRef-2.0 and Apache-2.0)', 17)
    token_assert_helper(lexer.token(), 'PKG_LICS_FFILE', 'PackageLicenseInfoFromFiles', 18)
    token_assert_helper(lexer.token(), 'LINE', 'Apache-1.0', 18)
    token_assert_helper(lexer.token(), 'PKG_LICS_FFILE', 'PackageLicenseInfoFromFiles', 19)
    token_assert_helper(lexer.token(), 'LINE', 'Apache-2.0', 19)
    token_assert_helper(lexer.token(), 'PKG_LICS_COMMENT', 'PackageLicenseComments', 20)
    token_assert_helper(lexer.token(), 'TEXT', '<text>License Comments</text>', 20)
    token_assert_helper(lexer.token(), 'PKG_EXT_REF', 'ExternalRef', 21)
    token_assert_helper(lexer.token(), 'LINE',
                        'SECURITY cpe23Type cpe:2.3:a:pivotal_software:spring_framework:4.1.0:*:*:*:*:*:*:', 21)
    token_assert_helper(lexer.token(), 'PKG_EXT_REF_COMMENT', 'ExternalRefComment', 22)
    ...
    token_assert_helper(lexer.token(), 'OPERATING_SYSTEM', 'OPERATING-SYSTEM', 23)
...
def test_tokenization_of_snippet(lexer):
    ...
    token_assert_helper(lexer.token(), 'SNIPPET_LICS_COMMENT', 'SnippetLicenseComments', 2)
    token_assert_helper(lexer.token(), 'TEXT', '<text>Some lic comment.</text>', 2)
    token_assert_helper(lexer.token(), 'SNIPPET_CR_TEXT', 'SnippetCopyrightText', 3)
    ...
    token_assert_helper(lexer.token(), 'SNIPPET_LICS_CONC',
                        'SnippetLicenseConcluded', 7)
    token_assert_helper(lexer.token(), 'LINE', 'Apache-2.0', 7)
    token_assert_helper(lexer.token(), 'SNIPPET_LICS_INFO', 'LicenseInfoInSnippet', 8)
...
def test_tokenization_of_annotation(lexer):
    ...
    token_assert_helper(lexer.token(), 'OTHER', 'OTHER', 4)
