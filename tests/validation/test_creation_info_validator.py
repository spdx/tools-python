...
@pytest.mark.parametrize \
        ("creation_info_input, expected_message",
         [(get_creation_info(spdx_version="version-2.3"),
           'spdx_version must be of the form "SPDX-[major].[minor]" but is: version-2.3'),
          (get_creation_info(spdx_id="SPDXRef-doc"),
           'spdx_id must be SPDXRef-DOCUMENT, but is: SPDXRef-doc'),
          (get_creation_info(data_license="MIT"),
           'data_license must be "CC0-1.0", but is: MIT'),
          (get_creation_info(document_namespace="some_namespace"),
...
def test_wrong_creation_info(creation_info_input, expected_message):
    ...
    expected = ValidationMessage(expected_message,
                                 ValidationContext("SPDXRef-DOCUMENT", None, SpdxElementType.DOCUMENT))
